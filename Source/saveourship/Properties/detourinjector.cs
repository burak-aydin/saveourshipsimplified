...
namespace saveourship
{
    ...
    internal static class DetourInjector
    {
        ...
        private static bool DoInject()
        {
            ...
            MethodInfo Verse_CountdownEnded = typeof(RimWorld.ShipCountdown).GetMethod("CountdownEnded", BindingFlags.NonPublic | BindingFlags.Static);
            MethodInfo saveourship_Verse_CountdownEnded = typeof(MyDetours).GetMethod("countdownend");
            if (!Detours.TryDetourFromTo(Verse_CountdownEnded, saveourship_Verse_CountdownEnded))
            {
